...
Calls an OpenAI-compatible ``/chat/completions`` endpoint (the Monad LiteLLM gateway in
front of gpt-oss-120b) and returns the final text. It calls LiteLLM **directly** — it does
not depend on Monad-1's MCP server (that was only the C0 spike vehicle).

Behaviour mirrors the C0 spike's ``_call_litellm`` (Monad-1 ``openclaw-monad-mcp``):
...
class OpenAICompatAdapter:
    ...
    def __init__(
        ...
        Args:
            base_url: OpenAI-compat base URL (e.g. ``http://monad-1...:4000/v1``).
