...
compaction interrupted between them leaves the rows counted twice, which is visible in the figure
and reconcilable against the rows still on disk. The opposite order would drop rows before anything
recorded them: a smaller number, no evidence, no recovery. Rows that stay are written back
byte-for-byte, so neither a field added by a newer version nor a line torn by an interrupted append
is lost to the rewrite. Nothing triggers compaction automatically yet — it is an explicit call
